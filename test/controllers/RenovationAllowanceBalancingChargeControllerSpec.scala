...
class RenovationAllowanceBalancingChargeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val renovationAllowanceBalancingChargeRoute = routes.RenovationAllowanceBalancingChargeController.onPageLoad(taxYear, NormalMode).url
  ...
  "RenovationAllowanceBalancingCharge Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(RenovationAllowanceBalancingChargePage, RenovationAllowanceBalancingCharge(true, Some(100.65))).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(RenovationAllowanceBalancingCharge(true, Some(100.65))),taxYear, NormalMode, "individual")(request, messages(application)).toString
      }
    }
