...
class CostOfServicesProvidedControllerSpec extends SpecBase with MockitoSugar {
  ...
  forAll(scenarios) { (isAgent: Boolean, agencyOrIndividual: String) => {
    ...
    s"CostOfServicesProvided Controller for $agencyOrIndividual" - {
      ...
      "must populate the view correctly on a GET when the question has previously been answered" in {
        ...
        val userAnswers = UserAnswers(userAnswersId).set(CostOfServicesProvidedPage, validAnswer).success.value
