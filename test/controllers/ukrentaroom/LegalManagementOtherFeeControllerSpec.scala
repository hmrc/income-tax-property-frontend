...
class LegalManagementOtherFeeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val legalManagementOtherFeeRoute = controllers.ukrentaroom.routes.LegalManagementOtherFeeController.onPageLoad(taxYear, NormalMode).url
  ...
  forAll(scenarios) { (isAgent: Boolean, agencyOrIndividual: String) => {
    ...
    s"LegalManagementOtherFee Controller for $agencyOrIndividual" - {
      ...
      "must populate the view correctly on a GET when the question has previously been answered" in {
        ...
        val userAnswers = UserAnswers(userAnswersId).set(LegalManagementOtherFeePage, validAnswer).success.value
