...
import forms.ukrentaroom.allowances.OtherCapitalAllowancesFormProvider
...
class RaROtherCapitalAllowancesControllerSpec extends SpecBase with MockitoSugar {
  ...
  forAll(scenarios) { (isAgent: Boolean, agencyOrIndividual: String) =>
    ...
    s"OtherCapitalAllowances Controller for $agencyOrIndividual" - {
      ...
      "must return OK and the correct view for a GET" in {
        ...
        running(application) {
          val fakeRequest = FakeRequest(GET, otherCapitalAllowancesRoute)
          val request = DataRequest(fakeRequest, "", user, emptyUserAnswers)
          val result = route(application, request).value

          val view = application.injector.instanceOf[OtherCapitalAllowancesView]
      ...
      "must populate the view correctly on a GET when the question has previously been answered" in {
        ...
        val userAnswers = UserAnswers(userAnswersId).set(RaROtherCapitalAllowancesPage, validAnswer).success.value
      ...
      "must redirect to the next page when valid data is submitted" in {
        ...
        running(application) {
          val request =
            FakeRequest(POST, otherCapitalAllowancesRoute)
              .withFormUrlEncodedBody(("otherCapitalAllowances", validAnswer.toString))
      ...
      "must redirect to Journey Recovery for a GET if no existing data is found" in {
        ...
        running(application) {
          val request = FakeRequest(GET, otherCapitalAllowancesRoute)
