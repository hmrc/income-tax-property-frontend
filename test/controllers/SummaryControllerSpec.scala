...
import pages.{UKPropertyPage, UKPropertySelectPage}
...
class SummaryControllerSpec extends SpecBase {
  ...
  "Summary Controller" - {
    ...
    "must display the property rentals section if property rentals is selected in the about section" in {
      ...
      val application = applicationBuilder(userAnswers = Some(userAnswersWithPropertyRentals), false).build()
    ...
    "must NOT display the property rentals section if property rentals is not selected in the about section" in {
      ...
      val application = applicationBuilder(userAnswers = Some(userAnswersWithoutPropertyRentals), false).build()
