...
class UkAndForeignPropertyAmountReceivedForGrantOfLeaseControllerSpec extends SpecBase with MockitoSugar {
  ...
  "GET" - {
    ...
    val answers = Seq(
      ...
      emptyUserAnswers
        .set(
          UkPropertyAmountReceivedForGrantOfLeasePage,
    ...
    userTypeAndAnswerCombinations.foreach { case (userType, agent, answer) =>
      s"Return OK for user type $userType and userAnswer is ${answer.get(UkPropertyAmountReceivedForGrantOfLeasePage)}" in {
        ...
        running(application) {
          ...
          val form = answer
            .get(UkPropertyAmountReceivedForGrantOfLeasePage)
