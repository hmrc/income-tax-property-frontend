...
class OtherPropertyIncomeControllerSpec extends SpecBase with MockitoSugar with ScalaFutures {

  def onwardRoute: Call = Call("GET", "/foo")

  val formProvider = new UkAndForeignPropertyOtherIncomeFromUkPropertyFormProvider()
  val form: Form[BigDecimal] = formProvider("individual")
  val taxYear: Int = 2024
  val otherIncomeFromUkProperty: BigDecimal = BigDecimal(12345)

  lazy val otherUkPropertyIncomeRoute: String =
    routes.OtherUkPropertyIncomeController.onPageLoad(taxYear, NormalMode).url

  "otherUkPropertyIncome Controller" - {
    ...
    "must return OK if the route is valid" in {
      ...
      running(application) {
        val request = FakeRequest(GET, otherUkPropertyIncomeRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        val request = FakeRequest(GET, otherUkPropertyIncomeRoute)
        val view = application.injector.instanceOf[OtherIncomeFromUkPropertyView]
        val result = route(application, request).value

        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(otherIncomeFromUkProperty), taxYear, NormalMode, "individual")(
          request,
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, otherUkPropertyIncomeRoute)
            .withFormUrlEncodedBody(("otherIncomeFromProperty", otherIncomeFromUkProperty.toString))
