...
class ForeignTaxDeductedFromDividendIncomeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ForeignTaxDeductedFromDividendIncome Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ForeignTaxDeductedFromDividendIncomePage, validAnswer).success.value
      ...
      running(application) {
        val request =
          FakeRequest(POST, foreignTaxDeductedFromDividendIncomeRoute)
            .withFormUrlEncodedBody(("foreignTaxDeductedFromDividendIncome", validAnswer.toString))
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, foreignTaxDeductedFromDividendIncomeRoute)
            .withFormUrlEncodedBody(("foreignTaxDeductedFromDividendIncome", "invalid value"))

        val boundForm = form.bind(Map("foreignTaxDeductedFromDividendIncome" -> "invalid value"))
