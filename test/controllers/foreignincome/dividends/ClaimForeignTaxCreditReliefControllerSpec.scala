...
import views.html.foreignincome.dividends.ClaimForeignTaxCreditReliefView
...
class ClaimForeignTaxCreditReliefControllerSpec extends SpecBase with MockitoSugar {
  ...
  val formProvider: ClaimForeignTaxCreditReliefFormProvider = new ClaimForeignTaxCreditReliefFormProvider()
  val form: Form[Boolean] = formProvider(isAgentMessageString)

  lazy val claimForeignTaxCreditReliefRoute = controllers.foreignincome.dividends.routes.ClaimForeignTaxCreditReliefController.onPageLoad(taxYear, countryCode, NormalMode).url

  "ClaimForeignTaxCreditRelief Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), taxYear, countryCode, isAgentMessageString, NormalMode)(request, messages(application)).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, claimForeignTaxCreditReliefRoute)
            .withFormUrlEncodedBody(("claimForeignTaxCreditRelief", ""))
