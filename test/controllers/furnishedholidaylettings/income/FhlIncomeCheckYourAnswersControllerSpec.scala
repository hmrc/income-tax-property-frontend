...
class FhlIncomeCheckYourAnswersControllerSpec extends SpecBase with SummaryListFluency {
  ...
  "FhlIncomeCheckYourAnswers Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.FhlIncomeCheckYourAnswersController.onPageLoad(taxYear).url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[FhlIncomeCheckYourAnswersView]
