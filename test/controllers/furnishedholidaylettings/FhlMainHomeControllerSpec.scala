...
class FhlMainHomeControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new FhlMainHomeFormProvider()

  val taxYear = 2024
  lazy val fhlMainHomeRoute = controllers.furnishedholidaylettings.routes.FhlMainHomeController.onPageLoad(taxYear, NormalMode).url
  ...
  forAll(scenarios) { (isAgency: Boolean, agencyOrIndividual: String) => {
    ...
    s"FhlMainHome Controller for $agencyOrIndividual" - {
      ...
      "must return OK and the correct view for a GET" in {
        ...
        running(application) {
          val fakeRequest = FakeRequest(GET, fhlMainHomeRoute)
          val request = DataRequest(fakeRequest, "", user, emptyUserAnswers)

          val result = route(application, request).value

          val view = application.injector.instanceOf[FhlMainHomeView]
      ...
      "must populate the view correctly on a GET when the question has previously been answered" in {
        ...
        val userAnswers = UserAnswers(userAnswersId).set(FhlMainHomePage, true).success.value
      ...
      "must redirect to the next page when valid data is submitted" in {
        ...
        running(application) {
          val request =
            FakeRequest(POST, fhlMainHomeRoute)
              .withFormUrlEncodedBody(("fhlMainHome", "true"))
      ...
      "must return a Bad Request and errors when invalid data is submitted" in {
        ...
        running(application) {
          val fakeRequest =
            FakeRequest(POST, fhlMainHomeRoute)
              .withFormUrlEncodedBody(("fhlMainHome", ""))
      ...
      "must redirect to Journey Recovery for a GET if no existing data is found" in {
        ...
        running(application) {
          val request = FakeRequest(GET, fhlMainHomeRoute)
