...
class ForeignPrivateUseAdjustmentControllerSpec extends SpecBase with MockitoSugar {

  val formProvider = new ForeignPrivateUseAdjustmentFormProvider()
  ...
  lazy val foreignPrivateUseAdjustmentRoute = controllers.foreign.adjustments.routes.ForeignPrivateUseAdjustmentController.onPageLoad(taxYear, countryCode, NormalMode).url

  "ForeignPrivateUseAdjustment Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, foreignPrivateUseAdjustmentRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[ForeignPrivateUseAdjustmentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ForeignPrivateUseAdjustmentPage(countryCode), validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, foreignPrivateUseAdjustmentRoute)
            .withFormUrlEncodedBody(("privateUseAdjustmentAmount", validAnswer.toString))
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, foreignPrivateUseAdjustmentRoute)
            .withFormUrlEncodedBody(("privateUseAdjustmentAmount", "invalid value"))

        val boundForm = form.bind(Map("privateUseAdjustmentAmount" -> "invalid value"))
