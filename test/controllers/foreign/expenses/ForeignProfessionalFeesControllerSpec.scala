...
class ForeignProfessionalFeesControllerSpec extends SpecBase with MockitoSugar {

  val formProvider = new ForeignProfessionalFeesFormProvider()
  ...
  lazy val foreignProfessionalFeesRoute = controllers.foreign.expenses.routes.ForeignProfessionalFeesController.onPageLoad(taxYear, countryCode, NormalMode).url

  "ForeignProfessionalFees Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, foreignProfessionalFeesRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[ForeignProfessionalFeesView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ForeignProfessionalFeesPage(countryCode), validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, foreignProfessionalFeesRoute)
            .withFormUrlEncodedBody(("foreignProfessionalFeesAmount", validAnswer.toString))
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, foreignProfessionalFeesRoute)
            .withFormUrlEncodedBody(("foreignProfessionalFeesAmount", "invalid value"))

        val boundForm = form.bind(Map("foreignProfessionalFeesAmount" -> "invalid value"))
