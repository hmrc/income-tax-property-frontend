...
class ForeignPropertyDetailsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ForeignPropertyDetails Controller" - {
    ...
    "must return OK and the correct view for an Individual" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.ForeignPropertyDetailsController.onPageLoad(taxYear).url)
        val result = route(application, request).value

        val view = application.injector.instanceOf[ForeignPropertyDetailsView]
    ...
    "must redirect to the overview if there is no result" in {
      ...
      running(application) {
        ...
        failure.getMessage mustBe "Foreign property details not found from 1171 API"
      }
    }
  }
}
