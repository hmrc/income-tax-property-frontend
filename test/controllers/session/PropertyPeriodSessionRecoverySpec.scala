...
class PropertyPeriodSessionRecoverySpec extends SpecBase with MockitoSugar with Fixture with Matchers {
  ...
  "PropertyPeriodSessionRecovery" - {
    ...
    "merged uk and foreign fetched data" in {
      ...
      propertyPeriodSessionRecovery.mergeFetchedData(ukFetchedData = Right(expectedUkFetchedPropertyData),
          foreignFetchedData = Right(expectedForeignFetchedPropertyData)) mustBe Right(expectedFetchedPropertyData)
    }
  }
}
