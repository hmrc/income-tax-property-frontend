...
class UKPropertySelectControllerSpec extends SpecBase with MockitoSugar {
  ...
  "UKPropertySelectController" - {
    ...
    "must return OK and the correct view for an Individual" in {
      ...
      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers), isAgent = false)
        .overrides(bind[AuthConnector].toInstance(authConnector))
        .build()

      running(application) {
        val request = FakeRequest(GET, ukPropertySelectRoute)
        val result = route(application, request).value
        val view = application.injector.instanceOf[UKPropertySelectView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(UKPropertyPage, UKPropertySelect.values.toSet).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, ukPropertySelectRoute)
            .withFormUrlEncodedBody(("value[0]", UKPropertySelect.values.head.toString))
