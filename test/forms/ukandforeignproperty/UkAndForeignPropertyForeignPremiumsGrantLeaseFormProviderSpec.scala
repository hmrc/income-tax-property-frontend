...
class UkAndForeignPropertyForeignPremiumsGrantLeaseFormProviderSpec extends BooleanFieldBehaviours with OptionValues {
      ...
      "when premiumsOfLeaseGrantAgreed is false" - {
        ...
        "and a non numeric value is entered then should fail to bind" in {
          ...
          boundForm.errors must contain(
            FormError(
              "premiumsOfLeaseGrant",
              s"ukAndForeignProperty.foreignPremiumsGrantLease.error.amount.nonNumeric.$individualOrAgent"
            )
          )
        }
        ...
        "and an amount is entered that is out of range then should fail to bind" in {
          ...
          boundForm.errors must contain(
            FormError(
              "premiumsOfLeaseGrant",
              "ukAndForeignProperty.foreignPremiumsGrantLease.error.outOfRange",
