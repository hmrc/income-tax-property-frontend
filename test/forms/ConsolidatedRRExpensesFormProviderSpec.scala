  ...
  "consolidatedRRExpensesAmount" - {
    "when consolidatedRRExpenses is true" - {
      "and an amount is entered, should successfully bind" in {
        val boundForm = form.bind(Map("consolidatedExpensesOrIndiv" -> "true", "consolidatedExpensesAmount" -> "12.34"))
        boundForm.value.value mustBe ConsolidatedRRExpenses(true, Some(12.34))
        boundForm.errors mustBe empty
      }

      "and no amount is entered, should fail to bind" in {
        val boundForm = form.bind(Map("consolidatedExpensesOrIndiv" -> "true"))
        boundForm.errors must contain(FormError("consolidatedExpensesAmount", "consolidatedRRExpenses.error.required.amount.individual"))
      }
    }
    "when consolidatedExpenses is false" - {
      "and an amount is entered, should successfully bind" in {
        val boundForm = form.bind(Map("consolidatedExpensesOrIndiv" -> "false", "consolidatedExpensesAmount" -> "1234"))
        boundForm.value.value mustBe ConsolidatedRRExpenses(false, None)
        boundForm.errors mustBe empty
      }
      "and no amount is entered, should successfully bind" in {
        val boundForm = form.bind(Map("consolidatedExpensesOrIndiv" -> "false"))
        boundForm.value.value mustBe ConsolidatedRRExpenses(false, None)
        boundForm.errors mustBe empty
      }
    }
