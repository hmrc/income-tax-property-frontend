...
class CalculatedFigureYourselfController @Inject() (
  ...
  def onPageLoad(taxYear: Int, mode: Mode, propertyType: PropertyType): Action[AnyContent] =
    (identify andThen getData andThen requireData) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(CalculatedFigureYourselfPage(propertyType)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <-
                Future.fromTry(request.userAnswers.set(CalculatedFigureYourselfPage(propertyType), value))
