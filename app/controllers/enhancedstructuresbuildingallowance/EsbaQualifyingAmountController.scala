...
class EsbaQualifyingAmountController @Inject() (
  ...
  def onPageLoad(taxYear: Int, index: Int, mode: Mode, propertyType: PropertyType): Action[AnyContent] =
    (identify andThen getData andThen requireData) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(EsbaQualifyingAmountPage(index, propertyType)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <-
                Future.fromTry(request.userAnswers.set(EsbaQualifyingAmountPage(index, propertyType), value))
