...
class ForeignReplacementOfDomesticGoodsController @Inject()(
                                        ...
                                        formProvider: ForeignReplacementOfDomesticGoodsFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: ForeignReplacementOfDomesticGoodsView
  ...
  def onPageLoad(taxYear: Int, countryCode: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ForeignReplacementOfDomesticGoodsPage(countryCode)) match {
        case None => form
  ...
  def onSubmit(taxYear: Int, countryCode: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ForeignReplacementOfDomesticGoodsPage(countryCode), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(foreignNavigator.nextPage(ForeignReplacementOfDomesticGoodsPage(countryCode), taxYear, mode, request.userAnswers, updatedAnswers))
      )
  }
}
