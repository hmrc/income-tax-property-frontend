...
class ForeignZeroEmissionCarAllowanceController @Inject()(
                                                           ...
                                                           formProvider: ForeignZeroEmissionCarAllowanceFormProvider,
                                                           val controllerComponents: MessagesControllerComponents,
                                                           view: ForeignZeroEmissionCarAllowanceView
  ...
  def onPageLoad(taxYear: Int, countryCode: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ForeignZeroEmissionCarAllowancePage(countryCode)) match {
        case None => form
  ...
  def onSubmit(taxYear: Int, countryCode: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ForeignZeroEmissionCarAllowancePage(countryCode), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(foreignNavigator.nextPage(ForeignZeroEmissionCarAllowancePage(countryCode), taxYear, mode, request.userAnswers, updatedAnswers))
      )
  }
}
