        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <-
                Future
                  .fromTry(request.userAnswers.set(ForeignStructureBuildingQualifyingAmountPage(countryCode), value))
              _ <- sessionRepository.set(updatedAnswers)
            } yield Redirect(
              navigator.nextPage(
                ForeignStructureBuildingQualifyingAmountPage(countryCode),
                taxYear,
                mode,
                request.userAnswers,
                updatedAnswers
