...
class ForeignPropertyRepairsAndMaintenanceController @Inject() (
  ...
  def onPageLoad(taxYear: Int, countryCode: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ForeignPropertyRepairsAndMaintenancePage(countryCode)) match {
        case None => form
  ...
  def onSubmit(taxYear: Int, countryCode: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ForeignPropertyRepairsAndMaintenancePage(countryCode), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(foreignNavigator.nextPage(ForeignPropertyRepairsAndMaintenancePage(countryCode), taxYear, mode, request.userAnswers, updatedAnswers))
      )
  }
}
