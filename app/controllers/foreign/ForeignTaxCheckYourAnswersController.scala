...
class ForeignTaxCheckYourAnswersController @Inject()(
                                                      ...
                                                      view: ForeignTaxCheckYourAnswersView)(implicit ec: ExecutionContext)
    ...
    (identify andThen getData andThen requireData) { implicit request =>
      val list = SummaryListViewModel(
        rows = Seq(
          ...
          ClaimForeignTaxCreditReliefSummary.row(taxYear, request.user.isAgentMessageKey, countryCode, request.userAnswers)
        ).flatten
      )
