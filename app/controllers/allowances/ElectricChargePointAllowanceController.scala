...
class ElectricChargePointAllowanceController @Inject()(
                                        ...
                                        formProvider: ElectricChargePointAllowanceFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: ElectricChargePointAllowanceView
  ...
  def onPageLoad(taxYear: Int, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ElectricChargePointAllowancePage) match {
        case None => form
  ...
  def onSubmit(taxYear: Int, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ElectricChargePointAllowancePage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(ElectricChargePointAllowancePage, taxYear, mode, request.userAnswers, updatedAnswers)
      ))
  }
}
