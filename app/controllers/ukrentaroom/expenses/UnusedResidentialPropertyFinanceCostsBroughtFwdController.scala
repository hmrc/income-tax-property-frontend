...
class UnusedResidentialPropertyFinanceCostsBroughtFwdController @Inject() (
  ...
  view: UnusedResidentialPropertyFinanceCostsBroughtFwdView
  ...
  def onPageLoad(taxYear: Int, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(UnusedResidentialPropertyFinanceCostsBroughtFwdPage) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <-
                Future.fromTry(request.userAnswers.set(UnusedResidentialPropertyFinanceCostsBroughtFwdPage, value))
              _ <- sessionRepository.set(updatedAnswers)
            } yield Redirect(
              navigator.nextPage(
                UnusedResidentialPropertyFinanceCostsBroughtFwdPage,
                taxYear,
                mode,
                request.userAnswers,
                updatedAnswers
