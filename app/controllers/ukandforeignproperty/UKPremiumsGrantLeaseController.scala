...
import pages.ukandforeignproperty.{UKPremiumsGrantLeasePage, UkPropertyAmountReceivedForGrantOfLeasePage, UkYearLeaseAmountPage}
...
class UKPremiumsGrantLeaseController @Inject() (
  ...
  def onPageLoad(taxYear: Int, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val receivedGrantLeaseAmount: Option[BigDecimal] = request.userAnswers.get(UkPropertyAmountReceivedForGrantOfLeasePage).map(_.amountReceivedForGrantOfLease)
