...
import pages.ukandforeignproperty.UkPropertyAmountReceivedForGrantOfLeasePage
...
class UkAndForeignPropertyAmountReceivedForGrantOfLeaseController @Inject()(
  ...
  def onPageLoad(taxYear: Int, mode: Mode): Action[AnyContent] = (identify andThen getData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.getOrElse(UserAnswers(request.userId)).get(UkPropertyAmountReceivedForGrantOfLeasePage) match {
        case None =>
          form
        case Some(value) =>
          form.fill(value)
  ...
  def onSubmit(taxYear: Int, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(UkPropertyAmountReceivedForGrantOfLeasePage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(UkPropertyAmountReceivedForGrantOfLeasePage, taxYear, mode, request.userAnswers, updatedAnswers))
      )
  }
}
