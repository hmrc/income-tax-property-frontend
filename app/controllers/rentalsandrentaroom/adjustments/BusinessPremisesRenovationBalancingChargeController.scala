...
import views.html.rentalsandrentaroom.adjustments.BusinessPremisesRenovationAllowanceBalancingChargeView

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class BusinessPremisesRenovationBalancingChargeController @Inject() (
  ...
  formProvider: BusinessPremisesRenovationBalancingChargeFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: BusinessPremisesRenovationAllowanceBalancingChargeView
    ...
    (identify andThen getData andThen requireData).async { implicit request =>
      ...
      form
        ...
        .fold(
          formWithErrors =>
            Future.successful(
              BadRequest(view(formWithErrors, taxYear, request.user.isAgentMessageKey, mode))
            ),
          value =>
            for {
              updatedAnswers <-
