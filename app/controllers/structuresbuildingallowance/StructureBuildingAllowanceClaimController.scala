...
class StructureBuildingAllowanceClaimController @Inject()(
  ...
  def onPageLoad(taxYear: Int, mode: Mode, index: Int): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(StructureBuildingAllowanceClaimPage(index, Rentals)) match {
        case None => form
  ...
  def onSubmit(taxYear: Int, mode: Mode, index: Int): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(StructureBuildingAllowanceClaimPage(index, Rentals), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(StructureBuildingAllowanceClaimPage(index, Rentals), taxYear, mode, index, request.userAnswers, updatedAnswers))
      )
  }
}
