...
object ForeignBalancingChargeSummary {

  def row(taxYear: Int, countryCode: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] = {
    answers.get(ForeignBalancingChargePage(countryCode)).flatMap {
      case BalancingCharge(true, amount) =>
        Some(SummaryListRowViewModel(
          key = KeyViewModel("foreignBalancingCharge.checkYourAnswersLabel").withCssClass(keyCssClass),
          value = ValueViewModel(bigDecimalCurrency(amount.get)).withCssClass(valueCssClass),
          actions = Seq(
            ActionItemViewModel("site.change", controllers.foreign.adjustments.routes.ForeignBalancingChargeController.onPageLoad(taxYear, countryCode, CheckMode).url)
              .withVisuallyHiddenText(messages("foreignBalancingCharge.change.hidden"))
          )))
