...
object ForeignStructuresBuildingAllowanceAddressSummary {
  ...
  def row(taxYear: Int, index: Int, answers: UserAnswers, countryCode: String)(implicit
    ...
    answers.get(ForeignStructuresBuildingAllowanceAddressPage(index, countryCode)).map { answer =>
      ...
      SummaryListRowViewModel(
        key = "foreignStructuresBuildingAllowanceAddress.checkYourAnswersLabel",
        value = ValueViewModel(HtmlContent(value)),
        actions = Seq(
          ActionItemViewModel(
            "site.change",
            routes.ForeignStructuresBuildingAllowanceAddressController
              .onPageLoad(taxYear, index, countryCode, CheckMode)
              .url
          )
            .withVisuallyHiddenText(messages("foreignStructuresBuildingAllowanceAddress.change.hidden"))
        )
      )
    }
}
