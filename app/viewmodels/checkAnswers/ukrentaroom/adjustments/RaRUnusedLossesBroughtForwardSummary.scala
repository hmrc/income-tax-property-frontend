...
import models.{CheckMode, RaRUnusedLossesBroughtForward, UserAnswers}
  ...
  def row(taxYear: Int, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(RaRUnusedLossesBroughtForwardPage).flatMap {
      answer =>
        ...
        answer match {
          case RaRUnusedLossesBroughtForward(true, Some(amount)) =>
            row(bigDecimalCurrency(amount))
          case RaRUnusedLossesBroughtForward(false, _) =>
            row("site.no")
          case _ =>
