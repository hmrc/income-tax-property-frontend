...
object ResidentialPropertyFinanceCostsSummary {
  ...
  def row(taxYear: Int, answers: UserAnswers, individualOrAgent: String)(implicit
    ...
    answers.get(ResidentialPropertyFinanceCostsRRPage).map { answer =>
      SummaryListRowViewModel(
        key = KeyViewModel(
          s"ukrentaroom.expenses.residentialPropertyFinanceCosts.checkYourAnswersLabel.$individualOrAgent"
        ).withCssClass(keyCssClass),
        value = ValueViewModel(bigDecimalCurrency(answer)).withCssClass(valueCssClass),
        actions = Seq(
          ActionItemViewModel(
            "site.change",
            ResidentialPropertyFinanceCostsRRController
