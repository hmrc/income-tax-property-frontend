...
object PropertyBusinessTravelCostsSummary {
  ...
  def row(taxYear: Int, answers: UserAnswers, propertyType: PropertyType)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PropertyBusinessTravelCostsPage(propertyType)) match {
      case Some(answer) =>
        Some(
          SummaryListRowViewModel(
            ...
            actions = Seq(
              ActionItemViewModel(
                "site.change",
                routes.PropertyBusinessTravelCostsController.onPageLoad(taxYear, CheckMode, propertyType).url
              )
