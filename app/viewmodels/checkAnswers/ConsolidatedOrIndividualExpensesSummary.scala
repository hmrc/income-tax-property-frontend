...
object ConsolidatedOrIndividualExpensesSummary  {
  ...
  def row(taxYear: Int, countryCode: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(ConsolidatedOrIndividualExpensesPage).map {
      answer =>

        val value = ValueViewModel(
          HtmlContent(
            HtmlFormat.escape(messages(s"consolidatedOrIndividualExpenses.$answer"))
          )
        )

        SummaryListRowViewModel(
          key     = "consolidatedOrIndividualExpenses.checkYourAnswersLabel",
          value   = value,
          actions = Seq(
            ActionItemViewModel("site.change", ConsolidatedOrIndividualExpensesController.onPageLoad(taxYear, countryCode, CheckMode).url)
