    ...
    answers.get(UkAndForeignPropertyRentalTypeUkPage).map {
      answers =>
        ...
        val value = ValueViewModel(
          HtmlContent(
            answers.map {
              answer => HtmlFormat.escape(messages(s"ukAndForeignPropertyRentalTypeUk.$answer")).toString
            }
            .mkString(",<br>")
          )
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.UkAndForeignPropertyRentalTypeUkController.onPageLoad(taxYear, CheckMode).url)
              .withVisuallyHiddenText(messages("ukAndForeignPropertyRentalTypeUk.change.hidden"))
          )
        )
    }
}
