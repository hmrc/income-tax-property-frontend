...
import controllers.ukandforeignproperty.routes.UkAndForeignPropertyForeignPremiumsGrantLeaseController
import models.ukAndForeign.UkAndForeignPropertyForeignPremiumsGrantLease
import models.{CheckMode, UserAnswers}
import pages.ukandforeignproperty.UkAndForeignPropertyForeignPremiumsGrantLeasePage
...
object ForeignPremiumsGrantLeaseSummary {
  def row(taxYear: Int, answers: UserAnswers)(implicit
    ...
    answers.get(UkAndForeignPropertyForeignPremiumsGrantLeasePage).flatMap {
      case UkAndForeignPropertyForeignPremiumsGrantLease(true, amount) =>
        Some(
          SummaryListRowViewModel(
            ...
            actions = Seq(
              ActionItemViewModel(
                "site.change",
                UkAndForeignPropertyForeignPremiumsGrantLeaseController.onPageLoad(taxYear, CheckMode).url
              )
                .withVisuallyHiddenText(messages("ukAndForeignProperty.foreignPremiumsGrantLease.change.hidden"))
            )
          )
        )
      case UkAndForeignPropertyForeignPremiumsGrantLease(false, amount) =>
        Some(
          SummaryListRowViewModel(
            key = KeyViewModel("ukAndForeignProperty.foreignPremiumsGrantLease.checkYourAnswersLabel").withCssClass(keyCssClass),
            value = ValueViewModel(bigDecimalCurrency(amount.get)).withCssClass(valueCssClass),
