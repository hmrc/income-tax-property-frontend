...
class ResidentialFinanceCostFormProvider @Inject() extends Mappings {
  ...
  def apply(individualOrAgent: String): Form[BigDecimal] =
    Form(
      "residentialFinanceCost" -> currency(
        ...
        .verifying(inRange(BigDecimal(0), BigDecimal(100000000), "residentialFinanceCost.error.outOfRange"))
    )
}
