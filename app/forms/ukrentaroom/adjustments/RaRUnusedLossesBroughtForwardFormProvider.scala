...
class RaRUnusedLossesBroughtForwardFormProvider @Inject() extends Mappings {
  ...
  def apply(individualOrAgent: String): Form[RaRUnusedLossesBroughtForward] =
    Form[RaRUnusedLossesBroughtForward](
      mapping(
        "raRUnusedLossesBroughtForwardYesOrNo" -> boolean(s"raRUnusedLossesBroughtForward.error.required.$individualOrAgent"),
        ...
        RaRUnusedLossesBroughtForward.unapply
      )
    )
}
