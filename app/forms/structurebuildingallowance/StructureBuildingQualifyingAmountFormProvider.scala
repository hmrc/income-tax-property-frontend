...
class StructureBuildingQualifyingAmountFormProvider @Inject() extends Mappings {
    ...
    Form(
      "structureBuildingQualifyingAmount" -> currency(
        ...
        "structureBuildingQualifyingAmount.error.nonNumeric")
          .verifying(inRange(BigDecimal(minimum), BigDecimal(maximum), "structureBuildingQualifyingAmount.error.outOfRange"))
    )
}
