...
class UkAndForeignPropertyOtherIncomeFromUkPropertyFormProvider @Inject() extends Mappings {
  ...
  def apply(individualOrAgent: String): Form[BigDecimal] =
    Form(
      "otherIncomeFromProperty" -> currency(
        s"ukAndForeignPropertyOtherIncomeFromProperty.error.required.$individualOrAgent",
        s"otherIncomeFromProperty.error.twoDecimalPlaces.$individualOrAgent",
        s"ukAndForeignPropertyOtherIncomeFromProperty.error.nonNumeric")
