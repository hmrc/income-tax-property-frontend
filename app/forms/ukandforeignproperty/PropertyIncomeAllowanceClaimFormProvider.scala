...
class PropertyIncomeAllowanceClaimFormProvider @Inject() extends Mappings {
  ...
  def apply(individualOrAgent: String): Form[BigDecimal] = {
    ...
    val maximumAllowance = BigDecimal(1000)
    ...
    Form(
      "propertyIncomeAllowanceClaimAmount" -> currency(
        ...
        .verifying(minimumValueWithCustomArgument(minimum, s"ukAndForeignPropertyIncomeAllowanceClaim.error.outOfRange", maximumAllowance))
        .verifying(maximumValue(maximumAllowance, s"propertyIncomeAllowanceClaim.error.maxCapped.${individualOrAgent}"))
    )
  }
}
