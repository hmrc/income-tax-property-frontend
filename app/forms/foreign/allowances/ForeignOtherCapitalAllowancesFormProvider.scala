...
class ForeignOtherCapitalAllowancesFormProvider @Inject() extends Mappings {
    ...
    Form(
      "otherCapitalAllowance" -> currency(
        ...
        s"foreignOtherCapitalAllowances.error.nonNumeric.$individualOrAgent")
          .verifying(inRange(BigDecimal(minimum), BigDecimal(maximum), "foreignOtherCapitalAllowances.error.outOfRange"))
    )
}
