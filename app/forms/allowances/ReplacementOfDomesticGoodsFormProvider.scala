...
class ReplacementOfDomesticGoodsFormProvider @Inject() extends Mappings {
    ...
    Form(
      "replacementOfDomesticGoodsAmount" -> currency(
        ...
        s"replacementOfDomesticGoods.error.nonNumeric.$individualOrAgent")
          .verifying(inRange(BigDecimal(minimum), BigDecimal(maximum), "replacementOfDomesticGoods.error.outOfRange"))
    )
}
