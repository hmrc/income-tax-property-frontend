...
class ConsolidatedRRExpensesFormProvider @Inject() extends Mappings {
  ...
  def apply(individualOrAgent: String): Form[ConsolidatedRRExpenses] =
    Form(
      mapping(
        "isConsolidatedExpenses" -> boolean(s"consolidatedExpenses.error.required.$individualOrAgent"),
      ...
      )(ConsolidatedRRExpenses.apply)(m => Some((m.isConsolidatedExpenses, m.consolidatedExpensesAmount)))
    )
}
