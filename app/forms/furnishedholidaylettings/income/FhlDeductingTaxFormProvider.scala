...
class FhlDeductingTaxFormProvider @Inject() extends Mappings {
  ...
  def apply(individualOrAgent: String): Form[DeductingTax] =
    Form[DeductingTax](
      mapping(
        "taxDeductedYesNo" -> boolean(s"fhlDeductingTax.error.required.$individualOrAgent"),
        "taxDeductedAmount" -> {
          mandatoryIfTrue("taxDeductedYesNo",
            currency(
              s"fhlDeductingTax.amount.error.required.$individualOrAgent",
