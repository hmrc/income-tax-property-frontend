...
object Addressable {
  ...
  def checkAddresses[T, U](first: T, other: U)(implicit addressableFirst: Addressable[T], addressableSecond: Addressable[U]): Boolean = {
    standardise(addressableFirst.getPostcode(first)).equals(standardise(addressableSecond.getPostcode(other))) &&
      standardise(addressableFirst.getBuildingName(first)).equals(standardise(addressableSecond.getBuildingName(other))) &&
      standardise(addressableFirst.getBuildingNumber(first)).equals(standardise(addressableSecond.getBuildingNumber(other)))
  }

  def get[A](index: Int)(implicit a: Addressable[A]): Gettable[A] = a.get(index)

  def getAddresses[T](index: Int, userAnswers: UserAnswers, addresses: List[T])(implicit a: Addressable[T], r: Reads[T]): List[T] = {
  ...
  implicit val sbaAddressable = new Addressable[StructuredBuildingAllowanceAddress] {
    ...
    override def get(index: Int): Gettable[StructuredBuildingAllowanceAddress] = StructuredBuildingAllowanceAddressPage(index)
  }

  implicit val esbaAddressable = new Addressable[EsbaAddress] {
