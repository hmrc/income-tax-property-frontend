...
case class ForeignSummaryPage(foreignCYADiversionService: ForeignCYADiversionService) {
  ...
  def foreignAdjustmentsItem(taxYear: Int, countryCode: String, isClaimPIA: Boolean, userAnswers: Option[UserAnswers]): TaskListItem = {
    val taskListTagForAdjustments = {
      ...
      isAdjustmentsComplete
        ...
        .getOrElse {
          ...
          val isIncomeSectionCompleted = userAnswers.flatMap(_.get(ForeignIncomeSectionCompletePage(countryCode))).contains(true)
          if (isPIA) {
            if (isIncomeSectionCompleted) {
              TaskListTag.NotStarted
            } else {
