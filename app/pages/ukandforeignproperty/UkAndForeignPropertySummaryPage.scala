...
import models.{Rentals, UkAndForeignPropertyRentalTypeUk, UserAnswers}
...
object UkAndForeignPropertySummaryPage {
  ...
  def apply(
    ...
    val foreignPropertyTaskListItems = foreignTaskList(taxYear, userAnswers, isClaimPIA, isClaimRelief, foreignCYADiversionService)
  ...
  def ukPropertyRentalTaskList(
    ...
    if (claimPIA) {
      Seq(
        ukSummary.propertyRentalsAdjustmentsItem(userAnswers, taxYear, TaskListTag.NotStarted)
      )
    } else {
      Seq(
        ...
        ukSummary.propertyRentalsAdjustmentsItem(userAnswers, taxYear, ukSummary.getTaskListTagStatus(userAnswers, Rentals))
      )
    }
  }
    ...
    val res = for {
      ...
      list =
        foreignCountries.map(c =>
          (c.name, getTaskListForForeignCountry(taxYear, claimPIA, claimRelief, c.code, userAnswers, foreignCYADiversionService))
        )
    } yield list
  ...
  def getTaskListForForeignCountry(
    ...
    (isRentalsSelected, isRentARoomSelected, claimRelief, claimPIA) match {
      ...
      case (false, true, Some(false), false) => foreignFullTaskList
          // Ref: MJ06
      case (false, true, Some(true), false) => Seq(
        foreignSummary.foreignTaxItem(taxYear, countryCode, userAnswers),
        foreignSummary.foreignIncomeItem(taxYear, countryCode, userAnswers),
        foreignSummary.foreignAllowancesItem(taxYear, countryCode, userAnswers),
        foreignSummary.foreignSBAItem(taxYear, countryCode, userAnswers),
