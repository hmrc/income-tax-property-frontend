...
object UkAndForeignPropertySummaryPage {
  ...
  def ukPropertyRentalTaskList(
    ...
    if (claimPIA) {
      Seq(
        ukSummary.propertyRentalsAdjustmentsItem(userAnswers, taxYear, Some(TaskListTag.NotStarted))
      )
    } else {
      Seq(
    ...
    (claimRelief, claimPIA) match {
      case (true, true) =>
        Seq(
          ...
          ukSummary.rentalsAndRaRAdjustmentsItem(taxYear, userAnswers, Some(TaskListTag.NotStarted))
        )
      case (true, false) =>
        Seq(
          ukSummary.rentalsAndRaRAboutItem(taxYear, userAnswers),
