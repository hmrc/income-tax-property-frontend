...
case class SummaryPage(cyaDiversionService: CYADiversionService) {
  ...
  def propertyRentalsAdjustmentsItem(
    userAnswers: Option[UserAnswers],
    taxYear: Int,
    taskListTagStatus: Option[TaskListTag.TaskListTag] = None
  ) =
    TaskListItem(
      "summary.adjustments",
      cyaDiversionService
      ...
      taskListTagStatus.getOrElse(getTaskListTagStatus(userAnswers, Rentals)),
  ...
  def rentalsAndRaRAdjustmentsItem(
    ...
    TaskListItem(
      ...
      taskListTagStatus.getOrElse(getTaskListTagStatus(userAnswers, RentalsRentARoom)),
